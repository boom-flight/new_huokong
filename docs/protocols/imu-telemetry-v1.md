...
固件通过 USART2 TX 以 115200 baud、8 data bits、no parity、1 stop bit 发送固定 32 字节二进制帧，调度频率为 200 Hz。多字节整数全部采用 little-endian。协议版本固定为 `1`，payload length 固定为 `26`。
...
| `30..31` | 2 | `crc16` | CRC 结果，little-endian。 |

浮点值缩放后按最接近整数舍入，并饱和到 `INT16_MIN..INT16_MAX`。非有限值编码为 `0`。角度 `180.0` 归一化为 `-180.0`。
...
- refout：`false`
- xorout：`0x0000`
- 覆盖范围：`frame[2..29]`，共 28 字节
- 标准校验向量：ASCII `123456789` 得到 `0x29B1`

同步字节 `frame[0..1]` 和 CRC 字段 `frame[30..31]` 不参与计算。黄金帧测试中的 CRC 字节为 `0x9B 0xAF`，即数值 `0xAF9B`。

## 调度与丢帧

遥测线程每 5 个 RT-Thread tick 唤醒一次；当前 tick 频率为 1000 Hz，因此目标频率为 200 Hz。线程使用两个 32 字节发送缓冲区，只有 DMA 成功排队后才切换缓冲区。
